...
For the two defining polynomials of the theory, `x³ − x − 1` (root ρ) and
...
Packaged as `pisot_boundary_dichotomy`: the 3D/lepton sector settles and the
4D/quark sector does not — a forced fact, not a chosen one.
...
/-- **The Pisot-boundary dichotomy — the settle/spiral fork of the theory is
forced.** From the two zero-parameter defining polynomials: the cubic's
transverse mode contracts (`‖z‖ < 1`, ρ settles / is Pisot) and the quartic's
expands (`‖w‖ > 1`, Q spirals / is non-Pisot). The 3D/lepton sector settles onto
its arithmetic; the 4D/quark sector does not. -/
